    ...
    Private Sub Hist_OnRender(ByVal Writer As System.Web.UI.HtmlTextWriter) Handles Hist.OnRender
        ...
        Try
            ...
            Else
                ...
                Writer.Write("<div class='PnlList'style='margin: 1px'>")
                ...
                For Each HItm In MbrHist.Hists
                    Writer.Write("<tr><th width='5%'>Date</th>")
                    Writer.Write("<td align='center'>")
                    Writer.Write(HItm.Date.ToShortDateString())
                    Writer.Write("</td><th width='15%'>Transaction</th><td>")
                    Writer.Write(HItm.Transaction)
                    Writer.Write("</td></tr></table>")

                    Writer.Write("<table><tr><th width='10%'>Amt</th><th width='15%'>Balance</th><th width='10%'>Check</th>")
                    Writer.Write("</tr>")
                    ...
                    Else
                        Writer.Write("&nbsp;")
                    End If
                    Writer.Write("</td></tr></table><table class='mobile' style='width: 100%'><tr style='width: 100%'><th width='25%'>Description</th><td align='left' class='alttd' style='width: 100%'>")
                    ...
                    Else
                        Writer.Write("&nbsp;")
                    End If
                    Writer.Write("</td></tr></table><table><tr>")
                    If (MbrHist.Acct.Type = TfEnumerators.EAcctType.Checking) Then
                        Writer.Write("<td class='viewCheck'>")
                        If (Not HItm.Check = 0) Then
                            If System.Web.Configuration.WebConfigurationManager.AppSettings.Item("ViewChecks") = "True" Then
                                Dim CheckImgPage As String = System.Web.Configuration.WebConfigurationManager.AppSettings.Item("ViewChecksPage")
                                Dim RTNu As String = System.Web.Configuration.WebConfigurationManager.AppSettings.Item("RT")
                                Writer.Write("<p>View &nbsp;")
                                Writer.Write("<a href=""" & CheckImgPage & "?acct=" & Hbk.FullCheckingAccountNumber(MbrHist.Acct.AcctNu.ToString()) & "&num=" & HItm.Check.ToString() & "&date=" & HItm.Date.ToShortDateString() & "&amt=" & System.Math.Abs(HItm.Amt).ToString("C").Replace("$", "").Replace(".", "").Replace(",", "") & "&FB=F&RT=" & RTNu & """ target=""_blank"">Front</a>")
                                Writer.Write("&nbsp;|&nbsp;")
                                Writer.Write("<a href=""" & CheckImgPage & "?acct=" & Hbk.FullCheckingAccountNumber(MbrHist.Acct.AcctNu.ToString()) & "&num=" & HItm.Check.ToString() & "&date=" & HItm.Date.ToShortDateString() & "&amt=" & System.Math.Abs(HItm.Amt).ToString("C").Replace("$", "").Replace(".", "").Replace(",", "") & "&FB=B&RT=" & RTNu & """ target=""_blank"">Back</a>")
                                Writer.Write("</p>")
                            End If
                        Else
                            Writer.Write("&nbsp;")
